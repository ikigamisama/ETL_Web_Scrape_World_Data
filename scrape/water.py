...
class Water(WorldStatistics):
    ...
    async def scrape_and_save(self):
        ...
        self.save_csv(country_water_final_df, "water/country_water.csv")
    ...
    async def _process_water_country_data(self, df):
        ...
        for i, row in df.iterrows():
            link = row['Link']
            Country = row['Country']
            country_water_soup = await scrape_url(link, ".not-prose")
            ...
            self.save_csv(water_country_df, f"water/country/{Country}.csv")
